...
const AGENTS: Agent[] = siteConfig.offices.map((office) => ({
  name: office.contactName,
  phone: office.phoneLabel,
  regionLabel: office.country.es,
  avatarInitials: office.contactName
    .split(' ')
    .map((part) => part[0])
    .join('')
    .slice(0, 2)
    .toUpperCase(),
  avatarBg: '#2B2B2B',
  rawPhone: office.whatsappNumber,
}));
...
export default function WhatsAppWidget() {
  ...
  const handleAgentClick = (agent: Agent) => {
    const defaultText = encodeURIComponent(t('whatsapp.defaultMessage'));
    const url = `https://wa.me/${agent.rawPhone}?text=${defaultText}`;
    ...
    <div className="whatsapp-widget-container" ref={widgetRef} id="whatsapp-widget">
      ...
      <div className={`whatsapp-card ${isOpen ? 'open' : ''}`}>
          ...
          <div className="whatsapp-header-info">
            <div className="whatsapp-header-logo-container">
              <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397.01 12.008.01c3.202.001 6.212 1.246 8.477 3.513 2.262 2.268 3.507 5.28 3.505 8.484-.004 6.657-5.34 11.997-11.953 11.997-2.005-.001-3.973-.502-5.724-1.457L0 24zm6.59-4.846c1.6.95 3.188 1.449 4.825 1.451 5.436 0 9.86-4.37 9.864-9.799.002-2.63-1.023-5.101-2.885-6.965C16.588 1.977 14.128.951 11.5.951c-5.438 0-9.863 4.37-9.866 9.801-.002 1.84.498 3.631 1.448 5.218L2.08 21.93l6.023-1.564zm10.745-6.721c-.266-.134-1.579-.779-1.823-.867-.245-.089-.422-.134-.599.134-.177.268-.687.867-.843 1.045-.156.177-.311.2-.577.067-.266-.134-1.122-.413-2.137-1.32-.789-.704-1.323-1.574-1.479-1.842-.156-.268-.017-.413.117-.547.12-.12.266-.312.4-.467.133-.156.177-.267.266-.445.089-.178.045-.334-.022-.467-.067-.134-.599-1.447-.822-1.98-.216-.52-.454-.45-.623-.458-.16-.008-.343-.01-.527-.01-.184 0-.485.069-.739.347-.253.278-1.037 1.012-1.037 2.47 0 1.457 1.06 2.868 1.208 3.067.148.199 2.088 3.193 5.06 4.478.707.305 1.258.487 1.688.625.711.226 1.357.194 1.868.118.57-.085 1.579-.646 1.8-.1.223.134.422.378.422.822 0 .178-.089.334-.177.467-.089.134-.266.2-.533.066z" fill="#fff" />
              </svg>
            </div>
            <div>
              <h4 className="whatsapp-header-title">{t('whatsapp.title')}</h4>
              <p className="whatsapp-header-status">
                <span className="whatsapp-status-pulse"></span>
        ...
        <div className="whatsapp-card-body">
          <p className="whatsapp-card-subtitle">{t('whatsapp.subtitle')}</p>
          <div className="whatsapp-agents-list">
            {AGENTS.map((agent) => (
              <button
                key={agent.rawPhone}
              ...
              >
                <div className="whatsapp-agent-avatar" style={{ backgroundColor: agent.avatarBg }}>
                  {agent.avatarInitials}
                  <span className="whatsapp-agent-online-badge"></span>
                </div>
                <div className="whatsapp-agent-details">
                  <span className="whatsapp-agent-name">{agent.name}</span>
                  <span className="whatsapp-agent-region">{agent.regionLabel}</span>
                  <span className="whatsapp-agent-phone">{agent.phone}</span>
                </div>
