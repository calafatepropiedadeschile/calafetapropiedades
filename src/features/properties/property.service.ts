...
export function createPropertyService(repository: PropertyRepository) {
  return {
    ...
    findPublishedBySlug(slug: string, locale: Locale = 'es'): Promise<Property | null> {
      return repository.findPublishedBySlug(slug, locale);
    },
...
export async function getStaticPropertyCatalog(locale: Locale = 'es'): Promise<PropertyCard[]> {
  return unstable_cache(
    async () => withDatabaseRole(
      'public',
      async (db) => createPropertyService(createPropertyRepository(db)).listStaticCatalog(locale)
    ),
...
export async function getPropertyBySlug(slug: string, locale: Locale = 'es'): Promise<Property | null> {
  return unstable_cache(
    async () => withDatabaseRole(
      'public',
      async (db) => createPropertyService(createPropertyRepository(db)).findPublishedBySlug(slug, locale)
    ),
...
export async function getFeaturedProperties(locale: Locale = 'es'): Promise<PropertyCard[]> {
  return unstable_cache(
    async () => withDatabaseRole(
      'public',
      async (db) => createPropertyService(createPropertyRepository(db)).listFeatured(locale)
    ),
